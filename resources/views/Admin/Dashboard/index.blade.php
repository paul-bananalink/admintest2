...
@section('content-child')
    ...
    <div class="mt-20 mb-20">
        <a href="{{route(Route::currentRouteName(), ['type' => \App\Services\DashboardService::TYPE_DAY])}}"
            @class([
                'dashboard-filter text-center',
                'active' => \App\Services\DashboardService::TYPE_DAY == request('type'),
            ])
            >
            <i class="fa fa-calendar left-calendar" aria-hidden="true"></i>일별
        </a>
        <a href="{{route(Route::currentRouteName(), ['type' => \App\Services\DashboardService::TYPE_MONTH])}}"
            @class([
                'dashboard-filter text-center',
                'active' => \App\Services\DashboardService::TYPE_MONTH == request('type')
            ])
            >
