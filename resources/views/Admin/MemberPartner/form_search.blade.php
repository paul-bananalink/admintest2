@php
    $all_url = route('admin.partner.indexLevel', ['level_type' => config('constant_view.PAGE_PARTNER.all.key')]);
        ...
        @if (url()->current() == route('admin.partner.index'))
            ...
            <div class="btn-group">
                <input placeholder="검색어입력" type="text"
                    class="form-control input-sm width-200 search-input-box h-33 p-l-5 text-white" name="input_search"
                    value="{{ request('input_search') }}">
            </div>
        @endif
        <div class="btn-group">
            @if (url()->current() == route('admin.partner.index'))
            ...
            @endif
            {{-- <a href="{{route('admin.partner.index')}}" @class(['btnstyle1-success' => url()->current() == route('admin.partner.index'), 'btnstyle1 height-30  btnstyle1-inverse2'])>
                파트너 트리
            </a>
            ...
            {{-- <a href="#" class="btnstyle1 height-30 btnstyle1-inverse2" data-target="#modalCreatePartner" data-toggle="modal">
                신규등록
            </a> --}}
        </div>
    </form>
