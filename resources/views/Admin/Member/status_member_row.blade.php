...
<tr class="member-row-{{ $member->mNo }} member-row-info-{{ $member->mNo }} @if ($member->mStatus != Member::M_STATUS_NINE) alpha-40 @endif"
    ...
    <td class="bg-black-darker" style="color: rgb(255, 255, 255)">
        <strong>{{ $member->partner->mPartnerName ?? '' }}</strong>
    </td>
    <td class="bg-black-darker p-0">
        {{ data_get($member, 'mRegDate') }}
        <br />
        <span class="text-gray">{{ data_get($member, 'mApproveRegDate') }}</span>
    </td>
    ...
    <td class="bg-black-darker">
        ...
        <x-common.row_info_money :member="$member" suffix="status-members-{{ $member->mNo }}" />
        <img src="{{ asset('refresh.png') }}" width="14" class="pull-right cursor refresh-member-row" data-id="{{ $member->mNo }}" />
    </td>
    <td class="bg-black-darker">
        @php
            $mNick = data_get($member, 'mNick');
            $mBankOwner = data_get($member, 'mBankOwner');
        @endphp
        {{ $mNick }}{{ $mBankOwner ? ' / ' . $mBankOwner : '' }}
    </td>    
    <td class="bg-black-darker">{{ data_get($member, 'mMemberID') }}</td>
    <td class="bg-black-darker">
    ...
    <td class="bg-black-darker">
        <span style="color: #5b8631" class="el-tooltip" aria-describedby="el-tooltip-2014"
    ...
    <td class="bg-black-darker width-40">
        <button type="button" data-toggle="collapse" data-target="#MEMBER_DETAIL{{ $member->mNo }}-status-members-{{ $member->mNo }}"
...
@if ($includeDetail)
    <tr class="m-0 p-0 height-0 member-row-{{ $member->mNo }} @if ($member->mStatus != Member::M_STATUS_NINE) alpha-40 @endif">
        <td colspan="16" class="m-0 p-0 bg-black-lighter">
            <div id="MEMBER_DETAIL{{ $member->mNo }}-status-members-{{ $member->mNo }}" class="collapse width-full member-detail"
