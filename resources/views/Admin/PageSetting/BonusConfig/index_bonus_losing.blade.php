        ...
        <div class="col-md-12">
            <div class="box">
                ...
                <div class="d-flex-space-between p-10">
                    <h3 class="cst_h3"><i class="fa fa-gear"></i> 낙첨 포인트</h3>
                    <div class="d-flex gap-10">
                        <x-common.toggle_switch_button content="사용" contentOff="미사용"
                            isCheck="{{ $data['is_available'] ?? false }}" id="losing_bonus-is_available"
                            name="losing_bonus-is_available"
                            urlAction="{{ route('admin.page-setting.bonus-config.toggle-json-field-bonus-config', [
                                'field' => 'losing_bonus[is_available]',
                                'bonusType' => \App\Models\BonusConfig::TYPE_LOSING_BONUS,
                            ]) }}"
                            width="100px" size="big" />
                        <button type="submit" form="bonus_config_index_bonus_losing"
                            class="btnstyle1-success btnstyle1 btnstyle1-sm height-30"><i class="fa fa-gear"></i> 설졍값
                            저장</button>
                    </div>
                </div>
                <form action="{{ route('admin.page-setting.bonus-config.updateBonusLosing') }}" method="POST"
                    id="bonus_config_index_bonus_losing">
                    @csrf
                                            ...
                                            @foreach (config('site_config.LEVELS') as $level)
                                                <td>
                                                    <input
                                                        ...
                                                        type="input"
                                                        name="losing_bonus[table][{{ $level }}][percent]"
                                            ...
                                            @foreach (config('site_config.LEVELS') as $level)
                                                <td>
                                                    <input
                                                        class="form-control width-full search-input-box height-33 text-white2 p-5"
                                                        type="input"
                                                        name="losing_bonus[table][{{ $level }}][max_payment_amount]"
                                    ...
                                    <x-common.setting_item title="쪽지" sub-title="">
                                        <div class="bg-black-2 col-md-12 height-full p-t-17 p-b-17">
                                            <input
                                                ...
                                                type="input" placeholder="제목" name="losing_bonus[note_title]"
                                                value="{{ $data['note_title'] ?? '' }}">
                                            <textarea name="losing_bonus[note_detail]" cols="30" rows="8" class="form-control" placeholder="내용">{{ $data['note_detail'] ?? '' }}</textarea>
                                        </div>
                                    ...
                                    <x-common.setting_item title="금액 표시 여부" sub-title="누적금액, 누적 포인트금액을 표시하는가?">
                                        <div class="bg-black-2 col-md-12 height-full p-t-17 p-b-17">
                                            <x-common.toggle_switch_button content="사용" contentOff="미사용"
                                                ...
                                                id="losing_bonus-is_display_the_amount" name="losing_bonus-is_display_the_amount"
                                                urlAction="{{ route('admin.page-setting.bonus-config.toggle-json-field-bonus-config', [
                                                    'field' => 'losing_bonus[is_display_the_amount]',
                                                    'bonusType' => \App\Models\BonusConfig::TYPE_LOSING_BONUS,
                                                ]) }}"
                                                size="big" />
                                        </div>
                                    ...
                                    <x-common.setting_item title="자동지급 사용여부(유저페이지 버튼노출)" sub-title="자동지급을 사용하는가?">
                                        <div class="bg-black-2 col-md-12 height-full p-t-17 p-b-17">
                                            <x-common.toggle_switch_button content="사용" contentOff="미사용"
                                                ...
                                                id="losing_bonus-is_use_auto_payment" name="losing_bonus-is_use_auto_payment"
                                                urlAction="{{ route('admin.page-setting.bonus-config.toggle-json-field-bonus-config', [
                                                    'field' => 'losing_bonus[is_use_auto_payment]',
                                                    'bonusType' => \App\Models\BonusConfig::TYPE_LOSING_BONUS,
                                                ]) }}"
                                                size="big" />
                                        </div>
